...
{ CAP-5 Pas2JS runtime proof (Windows x64):

    pweb://app/index.html + /assets/app.js (pas2js-compiled output of a real
    Pas2JS application consuming the pweb.native Pas2JS SDK)
      -> WebView2 WebResourceRequested handler (CAP-4W seam)
      -> TFolderAssetStore over the built frontend/dist
      -> Pas2JS -> pweb.native SDK -> __pweb_invoke -> scheduler
  ...
  the React example's host (examples/04-react) apart from the window
  title and log prefix: no backend file branches on frontend kind.

    pas2jsapp <path-to-frontend/dist> }
...
const
  ...
  APP_TITLE = 'PWeb CAP-5 Pas2JS';
  LOG_PREFIX = 'pas2jsapp';
...
begin
  ...
  try
    ...
    if ReportState = 1 then
      WriteLn(LOG_PREFIX,
        ': Pas2JS -> SDK -> scheduler -> mORMot -> 42 over pweb://app PASS')
    else
    begin
