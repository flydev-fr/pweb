...
- JSON is the control plane, the **reserved `_pweb/` prefix under `pweb://app`** the data plane — no base64 bulk over the bridge; large media is not an ordinary `app.pwb` asset in v1 (bundler enforces a size threshold; escape route is the blob plane). A blob is served from `pweb://app/_pweb/blob/<32 hex>`, **not** from `pweb://blob/<token>`: the invariant survives, its spelling does not. `PWEB_NATIVE_CSP` carries `connect-src 'self'`, `img-src 'self' data:` and `media-src 'self'`, an origin is scheme + host + port, and CAP-12A MEASURED on both reachable engines that a request for a second authority is refused by the CSP *before the handler is ever asked* — `second_authority_requests: 0` on a path that would have counted one. So the data plane lives inside the privileged origin or it does not exist (`_bmad-output/implementation-artifacts/cap12a-decision-artifact.md` §2). The prefix is reserved twice: every platform handler answers it before any asset store is consulted, and `pwebbundle` refuses a bundle that carries a first segment `_pweb`, with no override.
- The plane is **Range-based, not streaming-based**, and that is a measurement rather than a preference: WebView2 withholds a response body from the page until it is complete (one reader delivery of the whole 2 MiB, `fetch()` resolving 583 ms into a 640 ms production), so a streaming data plane is unbuildable as a promise on a target the product may not drop. `Range` is universal — the header reaches the handler and a 206 with `Content-Range` is honoured on all four targets — and a bounded ranged window **is** a whole body, which is why the macOS handler stays synchronous (CAP-12A §4, §6.5).
- `app.pwb` is ZIP read with `TZipRead`; PWB1/SynLZ only if a benchmark beats ZIP.
- **Trusted files are resolved from the running image, and the image path comes from the KERNEL** — `src/security/pweb.imagepath.pas` (`GetModuleFileNameW` / `readlink /proc/self/exe` / `_NSGetExecutablePath`+`realpath`), never `ParamStr(0)` or `Executable.ProgramFilePath`, whose RTL ANSI layer mangles a Windows path outside the active code page and stops an application installed under an accented directory from starting (CAP-10E). It is the ONE reader in shipped code and `test/cap10e/check_image_path.ps1` pins that. Two ratified consequences: on POSIX the path is the REAL image, so a host launched through a symlink resolves `app.pwb` beside the real executable and not beside the link; on Windows the loader's path is final — the helper follows no link and adds no `\\?\` prefix. An unanswerable kernel is a refusal, never a relative path.
