...
{ CAP-10D1: the driver that spawns the REAL `pweb build --profile` and, in
  its interrupt scenario, stops it mid-PACKAGING with a REAL interrupt.

  IT IS THE CAP-10D0 DRIVER'S TWIN, deliberately and almost line for line:
  the mechanism is the CAP-10C0 one (the child gets its own console, and on
  Windows the interrupt is delivered by test/cap10c0/pwebchild's
  `ctrlbreak <pid>`, which attaches to that console and raises
  CTRL_C_EVENT), and a second mechanism invented for a second driver would
  be a second thing to keep in step.

  WHAT IT MEASURES THAT pwebbuilddrv CANNOT. An interrupt during PACKAGING
  lands somewhere the ten stages never reach: after the release has been
  committed and verified, inside a child this shard added. The claim is
  therefore different and has to be measured rather than inherited - the
  previous artifacts untouched, no staging tree left, no descendant
  surviving, and a release directory that is still exactly what the build
  committed.

      pwebpackdrv --pweb <exe> --project <dir> --profile <name>
                  --report <file> [--helper <pwebchild>] [--cwd <dir>]
                  [--scenario interrupt|plain] [--stage <name>]

  interrupt  (default) wait until the named packaging step reports, let it
             settle, deliver ONE real interrupt, and let the supervisor run
             its ladder to the end.
  plain      run to completion and report what it printed. No signal is
             sent; this is the shape the W and L legs need, with the
             forwarded lines classified by stream rather than re-parsed out
             of a redirected file.

  --stage    which packaging step to interrupt. `iscc` on Windows, because
             it is the long child; `archive` is written in-process and has
             no child at all, so the POSIX legs use `package` and interrupt
             the supervisor between the release and the artifact.
...
var
  ...
  SummaryArtifact, SummarySha: RawUtf8;
  ProfileName: RawUtf8;
...
begin
  ...
  begin
    ...
    if (Scenario = scInterrupt) and
       ...
       (Copy(Line, 1, Length(StageName) + 8) =
          'pweb: ' + StageName + ': ') then
    begin
      Step := stSettle;
  ...
  if v <> '' then
    SummaryBytes := v;
  v := SummaryValue(Line, 'artifact');
  if v <> '' then
    SummaryArtifact := v;
  v := SummaryValue(Line, 'sha256');
  if v <> '' then
    SummarySha := v;
end;
...
begin
  ...
  //
  // Group AND Running are set deliberately, and both are facts rather than
  // guesses. The CAP-10C0 engine puts every supervised child at the head of
  // its OWN process group (docs/supervision-contract.md), so the pgid of the
  // `pweb` this driver spawned IS its pid; and a record whose Running is
  // False makes PWebCliChildStop a no-op, which is how this leg first
  // reported `interrupt_armed = true, interrupt_delivered = false`. Signaling
  // the GROUP is also what a real Ctrl+C does, so the fallback delivers the
  // same event the console mechanism does rather than a weaker one.
...
begin
  ...
  begin
    ...
    begin
      WriteLn(StdErr, 'pwebpackdrv: option without a value: ' + arg);
      Halt(2);
    end;
    Inc(i);
    if arg = '--pweb' then
    ...
    begin
      ...
      begin
        WriteLn(StdErr, 'pwebpackdrv: unknown scenario: ' + ArgAt(i));
        Halt(2);
      end;
    end
    else
    begin
      WriteLn(StdErr, 'pwebpackdrv: usage: --pweb <exe> --project <dir> ' +
        '--profile <name> --report <file> [--helper <p>] [--cwd <d>] ' +
        '[--scenario s] [--stage n]');
      Halt(2);
    end;
    Inc(i);
  ...
  if (PwebPath = '') or
     ...
     (ProfileName = '') then
  begin
    WriteLn(StdErr, 'pwebpackdrv: --pweb, --project, --profile and ' +
      '--report are required');
    Halt(2);
  end;
  if CwdPath = '' then
  ...
  begin
    WriteLn(StdErr, 'pwebpackdrv: precondition absent: --pweb');
    Halt(3);
  end;
  if PWebCliNodeKind(ProjectDir) = pcnMissing then
  begin
    WriteLn(StdErr, 'pwebpackdrv: precondition absent: --project');
    Halt(3);
  end;
  if PWebCliNodeKind(CwdPath) = pcnMissing then
  begin
    WriteLn(StdErr, 'pwebpackdrv: precondition absent: --cwd');
    Halt(3);
  end;
  ...
  begin
    WriteLn(StdErr, 'pwebpackdrv: supervision unavailable: no stop handler');
    Halt(3);
  end;
  ...
  Row('driver_profile', ProfileName);
  ...
  Row('summary_artifact', SummaryArtifact);
  Row('summary_sha256', SummarySha);
  ...
  begin
    WriteLn(StdErr, 'pwebpackdrv: could not write the report');
    Halt(3);
  end;
  ...
  PWebCliWriteNewFile(ReportPath + '.lines', RawByteString(Lines),
    {SetExecBit=}False);
  WriteLn(StdErr, 'pwebpackdrv: done (' +
