...
for artifact in build/cap10c3/bin/c3tests build/cap10c3/bin/pwebp2jdrv \
                ...
                build/cap10c3/dev-units/pweb.webview.devconsole.ppu; do
    [ -e "${artifact}" ] || die "expected ${artifact}"
done
for u in pweb.webview.devhost pweb.webview.devconsole; do
    if [ -e "build/cap10c3/release-units/${u}.ppu" ]; then
        die "the PAS2JS RELEASE unit set carries ${u} -- the development composition must be unreachable from a release build"
    fi
done
