...
{ Compile-level signature pin for ALL 17 entry points of the generated
  ...
  project depends on and is initialized with the address of the external
  function from pweb.lib.webview. FPC only accepts the initialization when
  parameter types, modifiers, result type, and calling convention match --
  so any drift in the generated binding breaks THIS COMPILE.
...
begin
  ...
  Bound := 0;
  if Pin_create <> nil then Inc(Bound);
  if Pin_destroy <> nil then Inc(Bound);
  if Pin_run <> nil then Inc(Bound);
  if Pin_terminate <> nil then Inc(Bound);
  if Pin_dispatch <> nil then Inc(Bound);
  if Pin_get_window <> nil then Inc(Bound);
  if Pin_get_native_handle <> nil then Inc(Bound);
  if Pin_set_title <> nil then Inc(Bound);
  if Pin_set_size <> nil then Inc(Bound);
  if Pin_navigate <> nil then Inc(Bound);
  if Pin_set_html <> nil then Inc(Bound);
  if Pin_init <> nil then Inc(Bound);
  if Pin_eval <> nil then Inc(Bound);
  if Pin_bind <> nil then Inc(Bound);
  if Pin_unbind <> nil then Inc(Bound);
  if Pin_return <> nil then Inc(Bound);
  if Pin_version <> nil then Inc(Bound);
