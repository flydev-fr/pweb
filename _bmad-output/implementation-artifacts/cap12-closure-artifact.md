...
**Verdict: MET for the read side.** Two clauses are recorded as deviations
rather than rounded up, and the write side is §3.
...
| R2 | MET | *"a service … answers with a `BlobHandle` envelope"* — the runtime-owned `pweb.fetch` service is the wired producer; `PWebBlobHandle` / `isPWebBlobHandle` in `@pweb/runtime` and `TPWebBlobHandle` in the Pas2JS SDK are the shipped type. `blob` is in every envelope and null in all but one case, so a caller never feature-detects it |
...
proof), and its one entry condition: a named consumer. `12B-2` stays ROADMAP,
owned by CAP-12C.
...
## 5. THE PHASE LEDGER — 13 ENTRIES, 0 ORPHANS
...
`test/backlog/dispositions.tsv`, and nothing else may.

| key | digest | verdict | owner | disposition |
|---|---|---|---|---|
| `12A-1` | `bf54f4e6` | UPSTREAM | WebKitGTK | the blob-backed request body fault; report written, range pinned at 2.52.6 on two hosts; the product's typed-array transport routes around it on its own merits |
| `12A-2` | `89b8a181` | ACCEPTED | nothing owed | WebView2 delivers no response body before it is complete, and drains bodies serially on the GUI thread; the plane is Range-based because of it |
| `12A-3` | `81963a97` | ACCEPTED | nothing owed | a WebKitGTK media element refuses a custom-scheme resource, so media by URL is typed per engine and never promised |
...
| `12B-1` | `4a03b815` | ACCEPTED | nothing owed | WKWebView delivers a `Blob`/`File`/`FormData` body as zero bytes, silently |
...
| `12-3` | `f8c9b6b5` | CLOSED | itself | the streaming promise removed and K9 inverted (§6) |
...
2 ROADMAP, 1 UPSTREAM. The backlog gate reads
**454 ledger entries, 0 orphans, 62 open** (4 fix-now, 57 roadmap, 1 upstream),
125 accepted, 267 closed.

### The rows CAP-12 owned in other phases

A closed phase owns nothing. Nine open rows named CAP-12 when 12B shipped; the
closure disposes of every one, and §5d now refuses an open row owned by
`CAP-12`.
...
| place | now says |
|---|---|
| `docs/cli-contract.md` §5, the SDKs paragraph | the long-poll is the receive loop, not a placeholder; the CAP-12A measurement; the data plane Range-based, not streaming-based; each parked receive holds one scheduler worker for up to `waitMs` |
...
| `sdk/typescript/src/socket.ts` | the same, in the header; the loop's own comment no longer names CAP-12 |
...
any of **34** files under `docs/`, `sdk/typescript/src`, `sdk/pas2js` and the
decorator, and requires the measurement's phrase in the four places that carried
it. On the unchanged tree it fired **eight** times — four promises, four missing
measurements — and nothing else in the contract gate moved; after the edits it
passes. CAP-5's zero-network sweep, which reads SDK comments too, passes on the
new text.
...
| what | old | new | why |
|---|---|---|---|
| CAP-15C K9 | requires "only thing that changes" in four files | refuses the CAP-12 streaming promise in 34 files; requires "Range-based, not streaming-based" in four | §6 |
| backlog shard map | 46 source specs | 47: `cap12-closure-artifact.md` → `12` | the closure's own entries |
| backlog gate | sections 1–5c | plus §5d: the closure table, and no open row owned by `CAP-12` | §5 |
| backlog negative self-test | 26 perturbations | **29** | a phase entry dropped from the closure table, a verdict that disagrees with the disposition table, an open row handed back to CAP-12 |
| backlog census | 450 entries; 63 open (58 roadmap); 123 accepted; 264 closed | **454**; **62** open (**57** roadmap); **125** accepted; **267** closed | four closure entries; `7M1-5` closed |
...
`git diff d46d6ad` over the frozen surface, for this closure's commit:
...
`test/cap15c/check_cap15c_contracts.ps1` (K9), `test/backlog/*`,
`deferred-work.md` (append only), this artifact, and the brief under
...
1. **One blob is one window.** A 40 MB document is not one handle in v1
   (§2 R3, `12B-3`).
2. **No upload.** The transport exists and is refused by name until a consumer
   arrives (§3, `12B-2`).
3. **No push.** Protocol v1 has none and the Range-based plane cannot supply
   one, so the socket door keeps its bounded long-poll, which holds a scheduler
   worker while it waits. Whether that starves other invocations under the
   ratified host defaults is a question this branch measures separately and
   records in its own ledger entry.
4. **Media by URL is not promised** (`12A-3`).
5. **`HEAD` is not served** (12B §11).
...
- The read side of the SPEC's line is met on four targets, with the URL
  spelling superseded by measurement and the size clause recorded as a
  deviation owned by `12B-3`.
...
- The phase ledger has 13 entries and no orphan, and no open row is owned by a
  closed phase.
