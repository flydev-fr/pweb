...
| D2-13 | c3483e49 | RESOLVED | closed on the honest route the entry named, as CAP-6 surface: `pwebbundle` reads its argv from GetCommandLineW through CommandLineToArgvW on Windows and ParamStr on POSIX, so no option, no exit code and no archive byte moved. Proven twice - a CAP-6 leg packs an accented input to an accented output and requires the archive to equal the ASCII-path one byte for byte, and CAP-10D2 gains CM7, a Windows-only leg that BUILDS a generated project at a spaced and accented root from the extracted SDK and requires the `pack` stage by name - so it runs exactly where this entry said it could not. CM7 stops before `run` for a measured reason it names: the generated APPLICATION resolves `app.pwb` through `Executable.ProgramFilePath`, the same RTL layer one level out and in shipped code, which is a separate user-facing defect with its own ledger entry. The divergence allowlist row for the bundler is re-ratified at 8 in the same commit |
