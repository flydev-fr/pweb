...
- A privileged WebView navigates only within `pweb://app/...` and **never navigates to external content**: every external navigation is cancelled, `https:` and `mailto:` included. Approved `https:`/`mailto:` URIs reach the operating system only through an explicit capability-authorized runtime invocation (`pweb.openExternal` -> `external.open`), never from a navigation hook and never from a gesture. External content never executes in a WebView holding the privileged bridge.
